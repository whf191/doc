...
while ($row = $result->fetch_assoc()) {
    ...
    echo "<p><a href=?fenlan_pk={$row['id']}> {$row['name']}({$row['heji']})</a> " .
    		"<span style='float:right;'><a href=/admin/admin.php?admin_fenlan_add_pk={$row['id']}>添加</a></span></p> ";
...
SELECT
	...
	neirong.wenzhang_fenlei_id_id as fenlan_id
FROM
	blog_wenzhang_fenlei AS fenlei
JOIN blog_wenzhang_neirong AS neirong ON fenlei.id = neirong.wenzhang_fenlei_id_id
WHERE
	neirong.daohanglan_id_id = 15 ORDER BY neirong.id desc
...
while ($row = $result->fetch_assoc()) {
    ...
    echo "<a href=/admin/wenzhang_chaxun.php?pk={$row['id']}> {$row['title']}</a>";
     echo "<span style='padding-right:50px;float:right;'><a href=/admin/admin.php?change=1&admin_fenlan_add_pk={$row['fenlan_id']}" .
     		"&pk={$row['id']}>" .
     		"修改</a>&nbsp;&nbsp;&nbsp;&nbsp;删除</span>";
